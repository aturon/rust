...
mod tests {
    ...
    iotest!(fn uid_fails_on_windows() {
        assert!(Command::new("test").uid(10).is_err());
    })
    ...
    pub fn pwd_cmd() -> Command {
        Command::new("/system/bin/sh").arg("-c").arg("pwd")
    }

    #[cfg(windows)]
    pub fn pwd_cmd() -> Command {
        Command::new("cmd").arg("/c").arg("cd")
    }
    ...
    #[cfg(target_os="android")]
    pub fn env_cmd() -> Command {
        Command::new("/system/bin/sh").arg("-c").arg("set")
    }

    #[cfg(windows)]
    pub fn env_cmd() -> Command {
        Command::new("cmd").arg("/c").arg("set")
    }
